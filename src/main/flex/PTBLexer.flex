...
%{
   ...
   * LexerFactory, and can specify the treatment of tokens by boolean
   ...
   * @param tf The LexerFactory that will be invoked to convert
   ...
   */
  public PTBLexer(Reader r, LexerFactory tf, String options) {
    this(r);
...
//      }
  ...
  private LexerFactory tokenFactory;
  private Object prevWord;
  ...
  private static void printDebug(String msg){
    if(debug_print == true) System.out.println(msg);
  }
...
FRAC = [\u00BC\u00BD\u00BE\u2153\u2154]|&(frac14|frac12|frac34);|(\p{N}{1,4}[- \u00A0])?\p{N}{1,4}(\\?\/|\u2044)\p{N}{1,4}
...
{ORDINALS}              { printDebug("ORDINALS" + yytext()); return getNext(); }
...
{SPMDASH}               { printDebug("SPMDASH" + yytext());if (ptb3Dashes) {
...
{SPAMP}                 { printDebug("SPAMP" + yytext());return getNormalizedAmpNext(); }
{SPPUNC}                { printDebug("SPUNC" + yytext());return getNext(); }
{WORD}/{REDAUX}         { printDebug("WORD/REDAUX" + yytext());final String origTxt =
yytext();
                                printDebug(origTxt);
...
{SWORD}/{SREDAUX}       { printDebug("SWORD/SREDAUX" + yytext());final String txt =
yytext();
                          return getNext(removeSoftHyphens(txt),
                                         txt); }
{WORD}                  { final String origTxt = yytext();
                            printDebug("WORD" + origTxt);
                          String tmp = removeSoftHyphens(origTxt);
                          if (americanize) {
                            tmp = Americanize.americanize(tmp);
                          }
...
{APOWORD}               { printDebug("APOWORD" + yytext());return getNext(); }
{APOWORD2}/[:letter:]   { printDebug("APOWORD2" + yytext());return getNext(); }
{FULLURL}               { printDebug("FULLURL" + yytext());String txt = yytext();
                          if (escapeForwardSlashAsterisk) {
                            txt = delimit(txt, '/');
                            txt = delimit(txt, '*');
                          }
                          return getNext(txt, yytext()); }
{LIKELYURL}             { printDebug("LIKELYURL" + yytext());String txt = yytext();
                          if (escapeForwardSlashAsterisk) {
                            txt = delimit(txt, '/');
                            txt = delimit(txt, '*');
                          }
                          return getNext(txt, yytext()); }
{EMAIL}                 { printDebug("EMAIL" + yytext());return getNext(); }
{TWITTER}               { printDebug("TWITTER" + yytext());return getNext(); }
{REDAUX}/[^A-Za-z]      { printDebug("REDAUX" + yytext());return handleQuotes(yytext(),
false);
                        }
{SREDAUX}/[^A-Za-z]     { printDebug("SREDAUX" + yytext());return handleQuotes(yytext(), false);
                        }
{DATE}                  { String txt = yytext();printDebug("DATE" + yytext());
...
{NUMBER}                { printDebug("NUMBER" + yytext());return getNext
(removeSoftHyphens(yytext()),
                                         yytext()); }
{SUBSUPNUM}             { return getNext(); }
<YyTraditionalTreebank3>{FRAC} { String txt = yytext(); printDebug("FRAC" + txt);
...
<YyStrictlyTreebank3>{FRACSTB3} { String txt = yytext();printDebug("FRACSTB3" + txt);
...
{FRAC2}                 { printDebug("FRAC2" + yytext()); return normalizeFractions(yytext())
; }
{TBSPEC}                { printDebug("TBSPEC" + yytext());return getNormalizedAmpNext()
; }
{BANGWORDS}     { printDebug("BANGWORDS" + yytext());return getNext(); }
{THING3}                { printDebug("THING3" + yytext());if
...
{DOLSIGN}               { printDebug("DOLSIGN" + yytext());return getNext(); }
{DOLSIGN2}              { printDebug("DOLSIGN2" + yytext());if (normalizeCurrency) {
...
{ABBREV3}/{SPACENL}?[:digit:]   {printDebug("ABBREV3" + yytext());
                          // try to work around an apparent jflex bug where it
                          // gets a space at the token end by getting
                          // wrong the length of the trailing context.
...
(pt[eyEY]|co)\./{SPACE}(ltd|lim)  { printDebug("pt" + yytext());return getNext(); }
{ABBREV1}/{SENTEND}     {printDebug("ABBREV1" + yytext());
                          ...
                          return getNext(s, yytext()); }
{ABBREV1}/[^][^]        { printDebug("ABBREV1 - " + yytext());return getNext(); }
...
{ABBREV2}               { printDebug("ABBREV2" + yytext());return getNext(); }
{ABBREV4}/{SPACE}       { printDebug("ABBREV3" + yytext());return getNext(); }
{ACRO}/{SPACENL}        { printDebug("ACRO" + yytext());return getNext(); }
{TBSPEC2}/{SPACENL}     { printDebug("TBSPEC2" + yytext());return getNext(); }
{FILENAME}/({SPACENL}|[.?!,])      { printDebug("FILENAME" + yytext());return getNext(); }
{WORD}\./{INSENTP}      { printDebug("WORD\\./" + yytext());return getNext
(removeSoftHyphens
(yytext()),
                                         yytext()); }
{PHONE}                 { printDebug("PHONE" + yytext());String txt = yytext();
                          if (normalizeSpace) {
                            // txt = SINGLE_SPACE_PATTERN.matcher(txt).replaceAll("\u00A0"); // change to non-breaking space
                            txt = txt.replace(' ', '\u00A0'); // change space to non-breaking space
                          }
...
{DBLQUOT}/[A-Za-z0-9$]  { printDebug("DBLQUOT" + yytext());return handleQuotes(yytext()
, true); }
{DBLQUOT}               { printDebug("DBLQUOT" + yytext());return handleQuotes(yytext(), false); }
\x7f                    { if (invertible) {
...
{FNMARKS}       { printDebug("FNMARKS" + yytext());return getNext(); }
{ASTS}          { printDebug("ASTS" + yytext());if (escapeForwardSlashAsterisk) {
                    return getNext(delimit(yytext(), '*'), yytext()); }
                  else {
                    return getNext();
                  }
                }
{INSENTP}       { printDebug("INSENTP" + yytext());return getNext(); }
[?!]+           { return getNext(); }
[.¡¿\u037E\u0589\u061F\u06D4\u0700-\u0702\u07FA\u3002]  { printDebug("[ ... ]" + yytext());return getNext(); }
=+               { printDebug("=" + yytext());return getNext(); }
\/              { printDebug("\\ /" + yytext());if (escapeForwardSlashAsterisk) {
                    return getNext(delimit(yytext(), '/'), yytext()); }
                  else {
                    return getNext();
                  }
...
/* {HTHING}/[^a-zA-Z0-9.+]    { return getNext(removeSoftHyphens(yytext()),
                                               yytext()); } */
{HTHING}\./{INSENTP}          { printDebug("HTHING dot INSENTP" + yytext());return getNext
(removeSoftHyphens
(yytext()),
                                               yytext()); }
{HTHING}        { printDebug("HTHING " + yytext());return getNext(removeSoftHyphens(yytext()), yytext()); }
{THING}\./{INSENTP}          { printDebug("THING dot INSENTP" + yytext());return getNext(); }
{THING}         { printDebug("THING" + yytext());return getNext(); }
{THINGA}\./{INSENTP}    { printDebug("THINGA dot INSENTP" + yytext());return
getNormalizedAmpNext()
; }
{THINGA}        { printDebug("THINGA dot INSENTP" + yytext());return getNormalizedAmpNext(); }
...
{REDAUX}        { printDebug("REDAUX" + yytext());return handleQuotes(yytext(),
false); }
{SREDAUX}       { printDebug("SREDAUX" + yytext());return handleQuotes(yytext(),
false); }
{QUOTES}        { printDebug("QUOTES" + yytext());return handleQuotes(yytext(),
false); }
{FAKEDUCKFEET}  { printDebug("FAKEDUCKFEET" + yytext());return getNext(); }
{MISCSYMBOL}    {  printDebug("MISCSYMBOL" + yytext());return getNext(); }
\0|{SPACES}|[\u200B\u200E-\u200F\uFEFF] { printDebug("\\0 SPACES" + yytext());if (invertible) {
                     prevWordAfter.append(yytext());
                  }
                }
{NEWLINE}       { printDebug("NEWLONE" + yytext());if (tokenizeNLs) {
                      return getNext(NEWLINE_TOKEN, yytext()); // js: for tokenizing carriage returns
                  } else if (invertible) {
                      prevWordAfter.append(yytext());
                  }
                }
&nbsp;          { printDebug("\\& nbsp" + yytext());if (invertible) {
                     prevWordAfter.append(yytext());
                  }
                }
.       { printDebug("EVERYTHINGELSE" + yytext());String str = yytext();
